import json
#import objectpath
def get_container_memory_bytes_used(filename, size, querying_factor):
    ...
    header = ["pod_id", "cluster_name", "container_name", "instance_id", "startTime","memory_type","doubleValue"]
    ...
    for metrics in time_series:
        ...
        if (container_name.startswith(querying_factor)):
            ...
            memory_type = metrics.get("metric").get("labels").get("memory_type")

            points = metrics.get("points")
            for point in points:
                startTime = point.get("interval").get("startTime")
                int64Value = point.get("value").get("int64Value")
                data = [pod_id,cluster_name,container_name, instance_id, str(startTime), memory_type, str(int64Value)]
